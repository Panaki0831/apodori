...
テキストにはWebサイト、ニュース記事、LinkedInプロフィールの情報が含まれる場合があります。

以下のJSON配列形式で出力してください:
[
  {
    "name": "氏名（日本語）",
    "name_romaji": "氏名のローマ字 (例: tanaka taro)",
    "title": "役職",
    "department": "所属部署",
    "email": "メールアドレス（判明している場合）",
    "linkedin_url": "LinkedInプロフィールURL（判明している場合）"
  }
]
...
- name_romajiは日本語名をヘボン式ローマ字で変換してください（例: 田中太郎 → tanaka taro）。
- LinkedInプロフィールのタイトルに英語名が含まれている場合はそれをname_romajiに使用してください。
